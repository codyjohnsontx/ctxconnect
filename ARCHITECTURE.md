...
the request when the account is missing, `active` is false, the session carries
no `signedInAt` claim at all, or the session was minted at or before the
account's `accessEndedAt` cutoff. Deactivating a staff account therefore cuts off
...
The missing-claim rule is the one with an operational consequence. Sessions minted
