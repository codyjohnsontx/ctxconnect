...
* When the dealership's day ends, and whose day a due date is counted on: [`src/lib/dealership-day.ts`](./src/lib/dealership-day.ts). A due date is an instant, and an instant is not a day until something picks a timezone. The **Command Center** due-today metric, the list behind it and the sweep that raises `FOLLOW_UP_DUE` all read the **dealership's** day - `DEALERSHIP_TIME_ZONE`, default `America/Chicago` - because a cron sweep has no viewer to ask. What the **viewer's** clock owns is the printed moment, and that is `LocalTimestamp` in a client component: a server render is UTC on Vercel, which printed a 10pm follow-up as due 3am the next day. The two disagree by design; `tests/dealership-day.test.ts` pins both, including a scan that fails any non-client file that formats a moment.
