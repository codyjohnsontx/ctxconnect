...
* What a canned reply may state as fact, and what it must ask for: [`src/lib/templates.ts`](./src/lib/templates.ts). A placeholder Attend cannot answer becomes a visible blank the advisor fills in, and the composer holds Send while one is there. That only works while the caller answers nothing on its behalf: `advisorName` defaulted to `"the team"` in the thread page, so an unassigned thread texted the customer a stand-in where a person's name belongs and Send stayed enabled. Pass `null` for a detail the thread does not have - as `unit` already did - rather than a word that reads like an answer. `tests/templates.test.ts` reads both routes to the composer out of the thread page - the derivation and every `advisorName={...}` it is handed on by - so a default put back on either one fails there rather than in a customer's text.
