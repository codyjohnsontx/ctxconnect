...
* Which message a queue row previews, and whose voice it is: [`src/lib/message-preview.ts`](./src/lib/message-preview.ts). The newest message that is **not** `Message.systemGenerated`, which is not always the newest message: a hand-off writes a note on every thread it moves, so a whole book handed over would otherwise arrive as rows all previewing the same sentence about the hand-off instead of what each customer last said. A note a person typed still previews - that is somebody's voice on the thread - and the label is what keeps it from reading as the customer's own words. Enforced by the `where` on the preview include in [`src/lib/data.ts`](./src/lib/data.ts), so the row still loads one message.
