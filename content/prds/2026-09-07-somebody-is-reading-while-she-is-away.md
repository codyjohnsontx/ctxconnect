...
* `AuditLog` - `coverage.start` and `coverage.end` on the account, and
  `conversation.coverageStart` / `coverageReturned` / `coverageKept` on each
  conversation. The account row answers "who covered whom and when"; the
  conversation rows answer "where did this thread go" for one thread months
  later.
