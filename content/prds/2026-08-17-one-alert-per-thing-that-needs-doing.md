...
- **A follow-up moved out of the due window keeps its alert.** The sweep reconciles a follow-up's two states only for tasks still due today or already late, so one moved past today would keep whichever alert it already had. Not reachable here: nothing in the product can change a due date - the only `task.update` sets `status` - so this is latent rather than live, and it belongs with the reschedule verb, which is separately tracked. Named because the line above would otherwise read as covering it.

- **Two writers still store the same fact in two shapes.** The Twilio webhook raises `UNASSIGNED_CONVERSATION` with the inbound message attached; the operational sweep raises it for the same thread with no message. Collapsing the key means the advisor now reads one alert either way, but making the two writers agree is an ingestion change and is filed separately. Closed on 2026-08-19: the columns that say what an alert is about are now built in one place, `notificationSubjectColumns` in `src/lib/notification-facts.ts`, and reached through the `NotificationSubject` type, so a writer that hangs a message on an alert that is about the thread no longer compiles. The webhook still records the text it was raised from, now named `raisedByMessageId`, which the key does not read. The writers' wording, priority and due date stay deliberately their own, because unifying those would change the rail's copy and its ordering. `tests/notification-write-shape.test.ts` fails if a third writer assembles a row itself. Why the raising text is kept rather than dropped: [the decision log](../decisions/2026-08-19-thread-alerts-keep-the-text-that-raised-them.md). Priority superseded on 2026-09-12: a writer supplies the rank of the thread or follow-up the alert is about and `notificationPriority` decides the alert's own, so the ordering change this bullet declined to make was made there on purpose; wording and due date are still the writer's. See [An Unowned Thread Ranks Where The Thread Ranks](./2026-09-12-an-unowned-thread-ranks-where-the-thread-ranks.md).
