...
- Follow-up due dates: a timezone bug here is invisible while the dev server stands in the same zone as the browser, which is the default locally and never true on Vercel. Restart the dev server under `TZ=UTC` with the browser left in another zone (`America/Chicago`) and reschedule a follow-up: a 16:45 pick has to store `21:45Z`. `rescheduleTask` refuses a bare local string outright, so a regression shows up as the action erroring rather than as a wrong hour - see `src/lib/follow-ups.ts`.
