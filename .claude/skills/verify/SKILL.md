...
- Follow-up due dates: a timezone bug here is invisible while the dev server stands in the same zone as the browser, which is the default locally and never true on Vercel. Restart the dev server under `TZ=UTC` with the browser left in another zone (`America/Chicago`) and create or reschedule a follow-up: a 16:45 pick has to store `21:45Z`, and a 00:30 pick has to stay on the day she picked. Both writers refuse a bare local string outright, so a regression shows up as the action erroring rather than as a wrong hour - see `src/lib/follow-ups.ts`. The picker's *default* is the other half: emulate a timezone far enough east that her evening is the server's next day (`Pacific/Auckland`) and check the create form still offers her own working hours.
