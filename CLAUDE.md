...
* Whether the brief's suggested follow-up already exists, and what a due date means: [`src/lib/follow-ups.ts`](./src/lib/follow-ups.ts). It also owns both ends of the picker-to-database trip - `instantFromDateTimeLocal` in the browser, `instantFromZonedIso` refusing anything at a server action that does not name its offset. `createTask` is the one writer still reading a bare local string, and is wrong on any server that is not in the advisor's timezone; do not copy it.
