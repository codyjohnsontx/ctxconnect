...
* Who is holding an advisor's conversations while she is away, and what comes back when she returns: [`src/lib/coverage.ts`](./src/lib/coverage.ts). Coverage moves the assignment, so everything downstream - the queue, the guard, `notifyAssignee` - keeps working unchanged, and `Conversation.coveredForUserId` records the advisor a thread goes back to. A coverage that is **starting** never overwrites that mark - the advisor already named on it is still away and still waiting - which is what makes chained coverage land each thread back with the advisor it actually belongs to. A coverage that is **ending** may rewrite it exactly once, and only for a thread that stays with a cover who is away herself: that thread is handed into her coverage (`coverageGoesBackTo`) rather than finalised onto the account standing in for her, which used to strand it with somebody who had never answered the customer. The return rule is the fragile half: it runs once per advisor per trip with nobody watching, so `coverageOutcome` is a pure function and the action loads only the coverage window (`createdAt >= coveredSince`) and lets it decide the rest. A reply that failed to send counts; an internal note does not. Background: [content/decisions/2026-09-07-a-thread-the-cover-answered-stays-with-the-cover.md](./content/decisions/2026-09-07-a-thread-the-cover-answered-stays-with-the-cover.md).
