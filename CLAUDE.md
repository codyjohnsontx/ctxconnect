...
* What an alert is about, when writing one: `NotificationSubject` and `notificationSubjectColumns`, in that same module and built from those same lists. Several writers raise the same alert - the Twilio webhook the moment a text lands, the operational sweep on every Command Center load - and they used to assemble the id columns each their own way, which is how one unowned thread came to be listed twice. The subject is now a type, so a message on an alert that is about the thread does not compile; the text such a row was raised from is `raisedByMessageId`, provenance the key does not read. The rail does read it - a copy quoting a customer text is shown over the sweep's generic one, and the latest text over an earlier one (`shownInstead`) - so it only ever names a text the customer sent, and a quote is built only by `quotedCustomerText`, which sets the words, that text and its time together for the webhook and the sweep alike, so a quoting copy is dated when the customer sent the text and not when the row was written. The sweep quotes an unowned thread's latest text through one batched statement for every such thread (`latestCustomerTextsQuery`), because it runs on every Command Center load; a Prisma `include` with a nested `take` binds no LIMIT and reads whole threads. Wording stays the writer's own; the rank does not, see below.
* How an alert ranks, and what keeps it ranked right: `notificationPriority` in that same module. Rank belongs to the fact, not to whoever writes it - the webhook hard-coded `Priority.HIGH` for an unowned thread where the sweep passed `conversation.priority`, so a LOW thread texted through the webhook was listed above every NORMAL alert on a rail that orders by priority and reads only `notificationScanLimit` rows. A writer no longer passes a rank at all: it passes `subjectPriority`, how the thread or follow-up behind the alert is ranked, and the rule turns that into the row's. Three types carry a floor of their own because the event is the severity (`SLA_MISSED`, `MESSAGE_FAILED`, `FOLLOW_UP_OVERDUE`): the alert ranks at the higher of that floor and its subject, never below its subject - a fixed rank that replaced the subject's dropped an URGENT follow-up to HIGH the moment it went late. `tests/notification-priority.test.ts` pins which are which, that no alert ranks below its subject, and scans the writers for a rank handed in as a constant. Agreeing at write time is only half of it, so `createIfMissingWithClient` also re-ranks every standing copy of a fact whose thread has been re-ranked since. Every copy means every copy: `sameFactNotificationsWhere` leaves the recipient out on purpose, because `notificationPriority` never reads the recipient and so two copies of one fact cannot legitimately rank differently. Both dimensions exist to reach a copy no writer returns to - one raised from an inbound text, because that text will not arrive again, and one held by a deactivated manager, whom the sweep no longer raises for and whose rows nothing resolves. Background: [content/decisions/2026-09-12-an-alert-ranks-where-its-subject-ranks.md](./content/decisions/2026-09-12-an-alert-ranks-where-its-subject-ranks.md).
* Who is holding an advisor's conversations while she is away, and what comes back when she returns: [`src/lib/coverage.ts`](./src/lib/coverage.ts). Coverage moves the assignment, so everything downstream - the queue, the guard, `notifyAssignee` - keeps working unchanged, and `Conversation.coveredForUserId` records the advisor a thread goes back to. That mark is written once and never overwritten, so a thread returns to the advisor it was **first** covered for: one that passes through a second coverage keeps the first mark, and the second cover's claim on it is recorded nowhere - a deliberate limitation, in the PRD's Open Questions ([content/prds/2026-09-07-somebody-is-reading-while-she-is-away.md](./content/prds/2026-09-07-somebody-is-reading-while-she-is-away.md)). The return rule is the fragile half: it runs once per advisor per trip with nobody watching, so `coverageOutcome` is a pure function and the action loads only the coverage window (`createdAt >= coveredSince`) and lets it decide the rest. Only the **cover's** own reply keeps a thread with her - a manager who answers once has spot-helped, not taken it on (changed 2026-09-08); a failed send still counts, an internal note does not. Background: [content/decisions/2026-09-07-a-thread-the-cover-answered-stays-with-the-cover.md](./content/decisions/2026-09-07-a-thread-the-cover-answered-stays-with-the-cover.md).
* Whether a customer has been left waiting, and what counts as seeing to them: [`src/lib/sla.ts`](./src/lib/sla.ts). A thread is attended when **anything** after the customer's last text is a reply **or an internal note a person typed** - any such row, not merely the newest one, because an advisor's answer stands whatever bookkeeping lands after it. A note Attend wrote itself is not - `Message.systemGenerated` marks those, and every site that writes one sets it: coverage starting, both endings, and ordinary hand reassignment, which is where this bug lived first. Narrowing the rule to replies instead is the cheap fix that throws away "called her, left a voicemail"; `tests/sla.test.ts` fails if anyone tries it. Attend's own notes are all built by `systemNote` in that same module and the test runs that constructor, so no call site can forget the mark - a note a person types stays outside it, unmarked, and goes on clearing the alert. Historical rows are unmarked and therefore read as human-written, deliberately - see the migration. Background: [content/decisions/2026-09-09-a-note-attend-wrote-itself-is-not-somebody-answering.md](./content/decisions/2026-09-09-a-note-attend-wrote-itself-is-not-somebody-answering.md).
* Whose voice a queue row is previewing: [`src/lib/message-preview.ts`](./src/lib/message-preview.ts). **Which** message it previews is decided in one other place and only there - the `where` on the preview include in [`src/lib/data.ts`](./src/lib/data.ts), which leaves out the notes Attend wrote itself. A hand-off writes one on every thread it moves, so without it a whole book handed over arrives as rows all previewing the same sentence about the hand-off instead of what each customer last said. A note a person typed is unmarked and still previews. Asked of the query rather than of the loaded row because the row is capped at one message, and a run of Attend's own notes has no bound to cap a skip-in-memory at.
* What a canned reply may state as fact, and what it must ask for: [`src/lib/templates.ts`](./src/lib/templates.ts). A placeholder Attend cannot answer becomes a visible blank the advisor fills in, and the composer holds Send while one is there. That only works while the caller answers nothing on its behalf: a detail the thread does not genuinely have is passed as `null`, never a word that reads like an answer. Every field of `TemplateContext` is nullable for that reason - `unit` with no vehicle linked, `advisorName` on an unassigned thread (it once defaulted to `"the team"`), `customerName` while the customer still carries the webhook's `Unknown 4821` placeholder (`isUnnamedCustomer` decides, in the thread page; the header and queue keep the label), and `dealershipName` until an admin has saved one (`getDealershipSettings` in `src/lib/data.ts` reports an absent row as absent rather than upserting the demo dealership's name). `tests/templates.test.ts` reads the derivations out of the thread page, pins every prop the composer is handed bare, and scans `data.ts` for a written dealership name, so a default put back on any of them fails there rather than in a customer's text.
* Whether a reply reached the customer: [`src/lib/message-delivery.ts`](./src/lib/message-delivery.ts). The queue row, the thread bubble and the composer banner all mark the same failure, and the row is the one that cannot see the thread - it loads a single message to preview, and that is the newest one a person wrote, in *any* direction. Ask the row's marker of the newest **reply**, never of the message it previews; a customer's text or a note somebody typed afterwards otherwise un-marks the row while the other two still warn.
...
Five habits this repo has paid for more than once:

* **A page under `(app)` is not re-guarded on a client-side navigation.** Next does not re-render a shared layout when moving between routes inside it, so a staff member already standing in the app whose access has ended reaches a layout-only-guarded page with nothing re-checking. Every page calls `requireUser()` itself; `tests/coverage.test.ts` scans the segment for it. The layout's call is a convenience, never the proof.
